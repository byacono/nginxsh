...
NGINX_CONTENT='https://github.com/puppetlabs/exercise-webpage/blob/master/index.html'
...
echo_usage() {
        ...
        echo -e ""
	echo -e "This script wil install and setup nginx."
...
test_pre() {
    ...
    grep -E '^(CentOS|Red Hat).*\s[56]+' -q /etc/redhat-release 1>/dev/null 2>&1 || error_exit "It doesn't look like we're on a supported system"
...
setup_nginx_repo() {
	# If repo definition exists, we will leave it alone.
...
enabled=1" >> ${NGINX_REPO}
	...
	then
		...
		( ${RPM_CMD} --import /tmp/nginx.key && echo_ok "Importing the nginx repo GPG key" ) || error_exit "Can't import nginx repo GPG key"
		rm -f /tmp/nginx.key
	fi
